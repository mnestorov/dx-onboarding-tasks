...
if ( ! class_exists( 'Wp_Rest_MultiplePostType_Controller' ) ) {
	 ...
	 *
	 * New filters:
	 *  - `rest_multiple_post_type_query`
	 *
	 * Filters the query arguments as generated from the request parameters.
	 *
	...
	class Wp_Rest_MultiplePostType_Controller extends \WP_REST_Controller {
		...
		public function get_items( $request ) {
			...
			// Set before into date query.
			// Date query must be specified as an array of an array.
			if ( isset( $request['before'] ) ) {
				$args['date_query'][0]['before'] = $request['before'];
			}

			// Set after into date query.
			// Date query must be specified as an array of an array.
			...
			foreach ( $query_result as $post ) {
				// Get $posts_controller for Post Type.
			...
			$max_pages = ceil( $total_posts / intval( $query_args['posts_per_page'] ) );

			// Construct response.
			$response = rest_ensure_response( $posts );
			$response->header( 'X-WP-Total', intval( $total_posts ) );
